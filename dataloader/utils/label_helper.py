...
class ScoreLabelHelper():
	...
	* Make more competemt score builder
	...
	def BuildLabels(self, mask, size):
		if size == None:
			large_score = torch.zeros([1, self.Lcorr, self.Lcorr])
			medium_score = torch.zeros([1, self.Mcorr, self.Mcorr])
			small_score = torch.zeros([1, self.Scorr, self.Scorr])
			return large_score, medium_score, small_score
		else:
